...
\begin{document}
  ...
  \section{Koide as a unit coefficient of variation}
    ...
    $\bar r = 17.716$, $\sigma_r = 17.715$, hence $\CV(\rl) = 0.99999$ and $Q_\ell = 0.666661$.
    The charged-lepton square-root mass vector has unit relative dispersion to five digits.
  ...
  \section{Separation between dispersion and hierarchy}
    ...
    The large hierarchy arises when one generation lies close to such a node: for the observed masses the electron
    phase sits about $2.6^\circ$ from the $135^\circ$ node, so $\sqrt{m_e}$ is nearly annulled by interference.
  ...
  \section{The cascade-ladder obstruction}
    ...
    The bound is a property of this specific pinned ladder, not of $\mathbb Z_2$-symmetric triples in general.
    It is stronger than the observation that the middle weight $xy$ carries no Veronese root: the carrier cannot
    generate unit dispersion for the square-root masses at all.
    The mass carrier committed by the sub-programme sharpens this.
    The projected Yukawa operator has hermitian square
    $H_\Pi = \lambda_Y^2 \operatorname{diag}(1, \tfrac12 + u, \tfrac12 - u)$ with a single overall norm
    $\lambda_Y$, and its CP-symmetric form is $H_\Pi^{1/2} = \lambda_Y \operatorname{diag}(1, \sqrt{\tfrac12 + u},
    \sqrt{\tfrac12 - u})$~\cite{Beau2026pyo}.
    At the dictionary value $u = 1/10$ (levels $1 : 3/5 : 2/5$), every consistent reading of $\sqrt m$ gives
    $\CV(\rl) \in [0.09, 0.37]$, far below unity, and $\lambda_Y$ cancels in the coefficient of variation, so the
    overall norm cannot lift it.
    Equivalently, the corpus variational principle that fixes the split --- Born--Infeld saturation~\cite{Beau2026bim}
    --- produces the ladder, whose dispersion class ($\CV \le 1/\sqrt2$) lies below the Koide target.
  ...
  \section{Maximum-entropy interpretation}
    ...
    \paragraph{The true lock: the discrete-to-continuous bridge.}
    Even granting the first-moment constraint, the value $\CV = 1$ does not follow for three discrete masses.
  ...
  \section{Status and open test}
    ...
    The cascade-ladder obstruction $\CV \le 1/\sqrt2$ is exact for the pinned ladder
    (Proposition~\ref{prop:ladder}), and the committed Yukawa carrier gives $\CV(\rl) \in [0.09, 0.37]$, so the
    observed unit dispersion is reached by no presently named mechanism of the projective cascade.
    ...
    separately pinned).
    A deeper lock sits below this constraint question: unit dispersion is a property of the continuous exponential,
    and three discrete masses do not inherit it (the discrete-weight extremum is uniform, giving $\CV = 0$), so a
    forced continuous-to-discrete bridge, not merely the constraint, is required for a derivation.
